...
function nextStepThought() {
    ...
    // Adicionando a atualização da saída também
...
function nextStepSaida() {
    ...
    // 2. Joga o texto no elemento correspondente da carta (ajuste a classe se o seu H3 for diferente)
...
// Calcule o limite máximo com base no total de itens (ex: 9 itens * 50px = 450px total)
// Subtraia a largura visível do container se necessário
const maxScroll = -(9 * itemWidth - (itemWidth * 5)); // Exemplo de limite

function nextSlide() {
    if (currentPosition > maxScroll) {
        currentPosition -= jumpStep; // Pula 250px para a esquerda
...
function prevSlide() {
    if (currentPosition < 0) {
        currentPosition += jumpStep; // Pula 250px para a direita
        const track = document.getElementById('track');
        if (track) {
            track.style.transform = `translateX(${currentPosition}px)`;
        }
...
// Dicionário com todos os SVGs usando os seus nomes exatos
const svgsEmocoes = {
...
// Função acionada ao clicar em qualquer carinha do carrossel
function selectEmotion(tipoEmocao) {
    let spanAlvo = document.getElementById('icone-resultado');
    
    if (spanAlvo && svgsEmocoes[tipoEmocao]) {
        // Joga exatamente o SVG correto lá dentro sem mexer em classes do Google
